...
with st.expander("📖 처음이신가요? 사용법 보기"):
    st.markdown(
        "**계약 전, 이 집이 안전한지 확인하세요.**\n\n"
        "**준비물** · 등기부등본 PDF (인터넷등기소 발급, 텍스트본) — *선택사항*\n\n"
        "**따라하기**\n"
        "1. **거래유형** — 전세·월세·매매 중 선택\n"
        "2. **집 찾기** — 아파트·빌라는 '단계 선택' 탭(시/도→시군구→동→단지), "
        "단독·다가구는 '직접 입력' 탭에서 주소 검색\n"
        "&nbsp;&nbsp;※ '단지 불러오기'는 처음 실행 시 실거래를 모으느라 시간이 다소 걸립니다.\n"
        "3. **건축물대장** — '건축물대장 조회' 클릭 (아파트·빌라는 동·호 선택)\n"
        "4. **시세 확인** — 아파트·빌라는 '실거래가 조회', "
        "단독·다가구는 '직접 입력'에 감정가·공시가 입력\n"
        "5. **등기부 올리기 (선택)** — 안 올려도 시세 기반 기본 진단은 됩니다. "
        "등기부는 근저당 등 선순위 채권(제한물권)을 확인하는 절차로, 올리면 자동 반영됩니다\n"
        "6. **결과 보기** — 보증금 입력 → 신호등으로 위험도 표시\n\n"
        "**신호등** · 🔴 위험 · 🟡 주의 · 🟢 안전 "
        "— 🟡·🔴이면 계약 전 전문가 상담을 권합니다.")
...
if items:
    ...
    if est:
        st.divider()
        st.markdown("**등기부등본 업로드 (선택사항)**")
        st.caption("등기부는 근저당 등 선순위 채권(제한물권)을 확인하는 절차입니다. "
                   "올리지 않아도 시세 기반 기본 진단은 되며, 올리면 채권최고액·위험을 자동 반영합니다.")
        pdf = st.file_uploader("등기부 PDF (선택)", type=["pdf"])
        if pdf is not None:
